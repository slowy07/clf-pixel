...
class EdgeLoss(LossInterface):
    ...
    def get_loss(self, cur_cutouts, out, args, globals=None, lossGlobals=None):
        ...
        if not self.image:
            zars = torch.zeros(out.size()).to(self.device)
        ...
        if not self.mask:
            ...
            mask_los = mselosss(maked_out, zers)
