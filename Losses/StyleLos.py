...
class Vgg16_Extractor(nn.Module):
    def __init__(self, space):
        super().__init__()
        self.vgg_layers = model.vgg16(pretrained=True).features

        for param in self.paramters():
            param.requires_grad = False
        self.capture_layer = [1, 3, 6, 8, 11, 13, 15, 22, 29]
        self.space = space

    def forward_base(self, x):
        feat = [x]
        for i in range(len(self.vgg_layers)):
            x = self.vgg_layer[i](x)
...
def tensor_resample(tensor, dst_size, mode="bilinear"):
    return F.interpolate(tensor, dst_size, align_corners=False)


def pil_resize_short_edge_to(pil, trg_size):
    short_w = pil.width < pil.height
    ar_resized_short = (trg_size / pil.height) if short_w else (trg_size / pil.width)
    reized = pil.resize(
...
def distmat(x, y, cos_d=True):
    if cos_d:
        M = pairwise_distance_cos(x, y)
    else:
        M = torch.sqrt(pairwise_distance_seq_l2(x, y))
...
def calculate_loss(
    ...
    feat_max = 3 + 2 * 64 + 128 * 2 + 256 * 3 + 512 * 2
...
def strotss_loss(
    out_tensor, style_tensor, content_weight=1.0 * 16.0, extractor=None
